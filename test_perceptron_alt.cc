...
unsigned int perceptron(const unsigned int, const unsigned int, double *[], unsigned char ** const);
...
unsigned int perceptron(const unsigned int width, const unsigned int height, double *w[], unsigned char ** const img){
	if (f<unsigned char>(width, height, w[0], img)){
		if (f<unsigned char>(width, height, w[1], img)){
			if (f<unsigned char>(width, height, w[2], img)){
				return f<unsigned char>(width, height, w[3], img) ? 0 : 4;
			}else{
				return f<unsigned char>(width, height, w[4], img) ? 6 : 9;
			}
		}else{
			return 8;
		}
	}else{
		if (f<unsigned char>(width, height, w[5], img)){
			if (f<unsigned char>(width, height, w[6], img)){
				return f<unsigned char>(width, height, w[7], img) ? 1 : 7;
			}else{
				return f<unsigned char>(width, height, w[8], img) ? 2 : 5;
			}
		}else{
			return 3;
		}
